...
const server = httpServer.createServer( (req, res) =>{ //Callback function
    ...
    if(query.id){// If there is query parameter named id
        ...
        if (pathname === '/' || pathname.toLowerCase() === '/loans') {
            ...
            //const inter = dataObj[Number(query.interest)];
            //const term = dataObj[Number(query.loanTermYears)]
            //const interst = dataObj[Number(query.interst)];
            //const numOfPeriods = dataObj[Number(query.loanTermYears)] * 12;
            const loanHTML = replaceTemplate(templateHTML, user);
            //let first = stuff(inter, term)
